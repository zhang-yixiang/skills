...
Every `SKILL.md` is either user-invoked (`policy.allow_implicit_invocation: false` in `agents/openai.yaml`, reachable only by the human) or model-invoked (model- or user-reachable). Codex-compatible `SKILL.md` frontmatter contains only `name` and `description`. See [.agents/invocation.md](./.agents/invocation.md).
